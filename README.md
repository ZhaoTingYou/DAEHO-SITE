...
- Desktop users see the video when present. Mobile, reduced-motion, and
  data-saver users see the responsive CMS poster instead.
...
- CMS-managed optimized video: `public/videos/daeho-home-hero.mp4`
...
- Font-face CSS is generated by `node scripts/sync-font-css.mjs` and bundled
  from `styles/vendor-fonts.css`; public and admin layouts must not add external
...
- Above-the-fold callers set `priority`; this emits mutually exclusive mobile
  and desktop preloads plus eager/high-priority image attributes.

...
test('page metadata prefers localized CMS SEO fields over page copy fallbacks', async () => {
  ...
  assert.equal(metadata.description, '대호는 우승반지, 임관반지, 단체 기념반지를 제작합니다. DAEHO 공식 사이트에서 확인하세요.');
  assert.equal(metadata.openGraph.images[0].url, '/images/seo-home.png');
  assert.deepEqual(globalThis.__seoGetPageCalls, ['home']);
});

test('Korean metadata normalizes brand words and keywords even when CMS copy omits them', async () => {
  globalThis.__seoPages = {
    'mastery-making': {
      seo: {
        ko: {
  ...
  assert.deepEqual(metadata.keywords.slice(0, 4), ['DAEHO', '대호', '우승반지 제작', '우승반지']);
  assert.ok(metadata.keywords.includes('임관반지'));
  assert.ok(metadata.keywords.includes('단체 맞춤 반지'));
});
...
test('home metadata fallback carries the DAEHO Korean brand name and core service', async () => {
  ...
  assert.equal(metadata.title, '대호 우승반지 제작 전문 | DAEHO');
  assert.equal(
    metadata.description,
    '대호 우승반지 제작은 1988년부터 이어 온 맞춤 반지 제작 경험을 바탕으로 프로스포츠 구단, 학교, 단체의 챔피언십 링과 기념반지를 완성합니다. DAEHO 공식 사이트에서 확인하세요.'
  );
  assert.equal(metadata.openGraph.siteName, '대호');
  assert.ok(metadata.keywords.includes('DAEHO'));
  assert.ok(metadata.keywords.includes('대호'));
});
...
test('default Korean metadata uses search-focused copy for priority service pages', async () => {
  ...
  assert.equal(collectionMetadata.title, '대호 우승반지·임관반지·주문제작 컬렉션 | DAEHO');
  assert.match(collectionMetadata.description, /대호/);
  assert.match(collectionMetadata.description, /우승반지/);
  assert.match(collectionMetadata.description, /임관반지/);
  ...
  assert.equal(techniqueMetadata.title, '대호 | 우승반지 제작 공정과 맞춤 세공 | DAEHO');

...
test('detail metadata resolves the default social image through Storage', async () => {
  globalThis.__seoEnglishEnabled = false;

  const seo = await importSeoWithStubs();
  const metadata = await seo.getDetailMetadata('ko', '/privacy', 'Privacy', 'Privacy policy');

  assert.equal(metadata.openGraph.images[0].url, '/images/home_hero.png');
  assert.deepEqual(metadata.twitter.images, ['/images/home_hero.png']);
});
...
test('home metadata fallback carries the DAEHO Korean brand name and core service', async () => {
  ...
  assert.equal(
    ...
    '대호(DAEHO)는 1988년부터 프로스포츠 구단, 학교, 단체의 우승반지와 기념반지를 맞춤 제작합니다.'
  );

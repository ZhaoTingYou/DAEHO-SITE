...
test('empty or non-array FIRST RECORDS use a cloned exact static locale fallback', () => {
  const {normalizeAchievementFirstRecordsFallback: normalize} = loadFirstRecordsCore();
...
test('configured FIRST RECORDS keep existing blank-image slots without padding missing records', () => {
  const {normalizeAchievementFirstRecords} = loadFirstRecordsCore();
  const result = normalizeAchievementFirstRecords([
    {frontTitle: '  CMS record  ', image: '  cms-record.png  '},
    {frontTitle: '  Missing image  ', image: '   '}
  ]);

  assert.deepEqual(plain(result), [
    {title: 'CMS record', image: 'cms-record.png'},
    {title: 'Missing image', image: ''}
  ]);
});

test('configured FIRST RECORDS use legacy title only when frontTitle is absent and cap output at four', () => {
  const {normalizeAchievementFirstRecords} = loadFirstRecordsCore();
  const result = normalizeAchievementFirstRecords([
    {frontTitle: '   ', title: 'Must stay hidden', image: 'one.png'},
    {title: '  Legacy title  ', image: 'two.png'},
    {frontTitle: 'Three', image: 'three.png'},
    {frontTitle: 'Four', image: 'four.png'},
    {frontTitle: 'Five', image: 'five.png'}
  ]);

  assert.deepEqual(plain(result), [
    {title: '', image: 'one.png'},
    {title: 'Legacy title', image: 'two.png'},
    {title: 'Three', image: 'three.png'},
    {title: 'Four', image: 'four.png'}
  ]);
});

...
test('profile provisioning is bound to the Cognito-verified Korean phone', () => {
  assert.deepEqual(
    profileProvisioningRequest('cognito-sub', {
      ...
      'cognito:username': 'Daeho.Member',
      registrationGrant: 'must-not-be-used-for-provisioning'
    }, 'server-cookie-grant'),
    {
      path: '/v1/internal/profiles/from-authenticated-phone',
      body: {
        subject: 'cognito-sub', phone: '+821012345678', loginName: 'daeho.member',
        registrationGrant: 'server-cookie-grant'
      }
    }
  );
  ...
  assert.deepEqual(profileProvisioningRequest('cognito-sub', {
    phone_number: '+821012345678',
    phone_number_verified: true,
    'cognito:username': 'generated-uuid-that-is-much-too-long'
  }, ''), {
    path: '/v1/internal/profiles/from-authenticated-phone',
    body: {subject: 'cognito-sub', phone: '+821012345678', loginName: '', registrationGrant: ''}
  });
});

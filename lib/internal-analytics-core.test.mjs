import assert from 'node:assert/strict';
import test from 'node:test';

import * as internalAnalytics from './internal-analytics-core.mjs';
import {
  INTERNAL_ANALYTICS_SESSION_TIMEOUT_MS,
  INTERNAL_ANALYTICS_STORAGE_KEY,
  classifyDevice,
  classifyTrafficSource,
  resolveSessionState,
  sanitizeReferrerHost
} from './internal-analytics-core.mjs';

const seed = {sessionId: '00000000-0000-4000-8000-000000000002'};

test('classifies campaign and referrer traffic with UTM priority', () => {
  assert.deepEqual(
    classifyTrafficSource({
      source: 'instagram',
      medium: 'social',
      campaign: 'always_on',
      content: 'profile_link',
      referrerHost: 'google.com'
    }),
    {
      channel: 'instagram',
      source: 'instagram',
      medium: 'social',
      campaign: 'always_on',
      content: 'profile_link'
    }
  );
  assert.equal(
    classifyTrafficSource({source: 'newsletter', medium: 'email', referrerHost: 'google.com'}).channel,
    'other'
  );
  assert.equal(
    classifyTrafficSource({source: '', medium: 'social', referrerHost: 'google.com'}).channel,
    'social'
  );
  assert.deepEqual(
    classifyTrafficSource({source: '', medium: '', referrerHost: 'search.naver.com'}),
    {channel: 'naver', source: '', medium: '', campaign: '', content: ''}
  );
  assert.deepEqual(
    classifyTrafficSource({source: '', medium: '', referrerHost: ''}),
    {channel: 'direct', source: '', medium: '', campaign: '', content: ''}
  );
});

test('sanitizes referrers to external hostnames only', () => {
  assert.equal(
    sanitizeReferrerHost('https://www.google.com/search?q=private', 'https://daeho.works/ko'),
    'google.com'
  );
  assert.equal(sanitizeReferrerHost('https://daeho.works/private?name=secret', 'https://daeho.works'), '');
  assert.equal(sanitizeReferrerHost('not a url', 'https://daeho.works'), '');
});

test('sanitizes internal paths to UTM attribution without click identifiers or fragments', () => {
  const {sanitizeInternalAnalyticsPath} = internalAnalytics;

  assert.equal(typeof sanitizeInternalAnalyticsPath, 'function');
  assert.equal(
    sanitizeInternalAnalyticsPath(
      'https://daeho.works/en/golf?gclid=google-click&utm_id=campaign-id&utm_source=instagram&utm_medium=social&dclid=display-click&utm_campaign=always_on&utm_source_platform=instagram_ads&utm_term=custom_audience&utm_content=story&message=private#details'
    ),
    '/en/golf?utm_id=campaign-id&utm_source=instagram&utm_medium=social&utm_campaign=always_on&utm_source_platform=instagram_ads&utm_term=custom_audience&utm_content=story'
  );
});

test('classifies device from the mobile hint first and viewport second', () => {
  assert.equal(classifyDevice(1200, true), 'mobile');
  assert.equal(classifyDevice(375, false), 'mobile');
  assert.equal(classifyDevice(768, false), 'tablet');
  assert.equal(classifyDevice(1023, false), 'tablet');
  assert.equal(classifyDevice(1024, false), 'desktop');
});

test('reuses a session for 30 minutes and rotates it afterwards', () => {
  const stored = {sessionId: '00000000-0000-4000-8000-000000000001', lastActivityAt: 1_000};
  assert.equal(resolveSessionState(stored, 1_000 + 29 * 60_000, seed).isNew, false);
  assert.equal(resolveSessionState(stored, 1_000 + 30 * 60_000 + 1, seed).isNew, true);
  assert.equal(INTERNAL_ANALYTICS_SESSION_TIMEOUT_MS, 30 * 60 * 1000);
});

test('rejects malformed stored sessions and uses the injected session id', () => {
  const result = resolveSessionState({sessionId: 'not-a-uuid', lastActivityAt: 'old'}, 10_000, seed);

  assert.deepEqual(result, {
    sessionId: seed.sessionId,
    lastActivityAt: 10_000,
    isNew: true
  });
  assert.equal(INTERNAL_ANALYTICS_STORAGE_KEY, 'daeho_internal_analytics_session_v1');
});

test('rotates a session when its stored activity timestamp is in the future', () => {
  const result = resolveSessionState(
    {sessionId: '00000000-0000-4000-8000-000000000001', lastActivityAt: 20_000},
    10_000,
    seed
  );

  assert.deepEqual(result, {
    sessionId: seed.sessionId,
    lastActivityAt: 10_000,
    isNew: true
  });
});

...
const INTERNAL_UTM_PARAMETERS = new Set([
  'utm_id',
  'utm_source',
  'utm_medium',
  'utm_campaign',
...
export function sanitizeInternalAnalyticsPath(input) {
  const url = new URL(input, 'https://daeho.works');
  const search = new URLSearchParams();

  for (const [key, value] of url.searchParams) {
    if (INTERNAL_UTM_PARAMETERS.has(key)) {
...
export function classifyTrafficSource(input = {}) {
  ...
  const channel =
    ...
    candidate === 'qr' ? 'qr' :
    hasUtm && medium.toLowerCase() === 'social' ? 'social' :
    hasUtm ? 'other' :
...
export function resolveSessionState(storedState, nowMs, seed = {}) {
  ...
  const validStoredState = isValidStoredState(storedState, nowMs);
...
function isValidStoredState(value, currentNowMs) {
  return Boolean(
    value &&
    typeof value === 'object' &&
    ...
    value.lastActivityAt <= currentNowMs
  );
}

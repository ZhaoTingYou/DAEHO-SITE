...
test('home production video section sits between stats and latest news', () => {
  ...
  assert.match(homePageSource, /src="\/videos\/home2\.mp4"/);
  assert.match(homePageSource, /poster="\/images\/home2_video_poster\.jpg"/);
...
test('News card cover stays card-only while SEO still uses the same effective image', () => {
  ...
  assert.match(publicContentSource, /ogImagePath: cmsImageName\(cmsItem\.ogImagePath \|\| card\.image\)/);
...
test('News article editor exposes ordered detail blocks per locale', () => {
  assert.match(newsFormSource, /NewsBlocksEditor/);
  assert.match(newsBlocksEditorSource, /type NewsBodyBlock/);
  assert.match(newsBlocksEditorSource, /type: 'text' \| 'imageFull' \| 'imageText' \| 'quote'/);
...
test('News detail renders CMS body blocks before falling back to legacy paragraphs', () => {
  ...
  assert.match(newsDetailPageSource, /detail\.blocks\.length > 0/);
  assert.match(newsDetailPageSource, /NewsLegacyBody/);

...
test('home news popup no longer renders article body copy', () => {
  assert.doesNotMatch(homeNewsPopupsSource, /splitModalBody/);
  assert.doesNotMatch(homeNewsPopupsSource, /activeBodyParagraphs/);
});
...
test('News article editor exposes ordered detail blocks per locale', () => {
  ...
  assert.doesNotMatch(newsFormSource, /form\.ctaTitle/);
  ...
  assert.match(newsFormSource, /imageGuide=\{t\('imageGuide\.newsCover'\)\}/);
  assert.match(newsFormSource, /imageGuide=\{t\('imageGuide\.seo'\)\}/);

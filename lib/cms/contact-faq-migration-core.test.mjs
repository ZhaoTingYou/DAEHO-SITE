...
test('remains idempotent after PostgreSQL jsonb reorders object keys', () => {
  const migrated = migrateContactFaqContents({
    ko: groupedContent(koContact.faqs),
    en: groupedContent(enContact.faqs)
  }, canonical);
  const jsonbRoundTrip = sortObjectKeys(migrated.content);

  const repeated = migrateContactFaqContents(jsonbRoundTrip, canonical);

  assert.equal(repeated.changed, false);
  assert.deepEqual(repeated.content, jsonbRoundTrip);
});

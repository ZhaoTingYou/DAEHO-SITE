...
test('Nginx overwrites the internal client IP header before proxying to Next', () => {
  for (const configPath of ['docker/nginx/default.conf', 'docker/nginx/https.conf']) {
    const source = readFileSync(path.join(repoRoot, configPath), 'utf8');
    const nextLocations = source.match(/location \/ \{[\s\S]*?\n  \}/g) ?? [];

    assert.ok(nextLocations.length > 0, configPath);
    for (const location of nextLocations) {
      assert.match(location, /proxy_set_header X-Daeho-Client-IP \$remote_addr;/);
    }
  }
});

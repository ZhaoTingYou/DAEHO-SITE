...
const legacyRequiredTables = cmsImportTables.slice(0, 8);
...
export function validateCmsImportSnapshot(snapshot) {
  ...
  for (const table of cmsImportTables) {
    ...
    if (rows === undefined && !legacyRequiredTables.includes(table)) {
      continue;
    }
...
export function getCmsImportCounts(snapshot) {
  ...
  return cmsImportTables.map((table) => ({
    table,
    count: normalizeImportRows(table, snapshot.tables[table] ?? []).length
  }));
}
...
export function importCmsSnapshot(db, snapshot) {
  ...
  const importTransaction = db.transaction(() => {
    ...
    for (const table of cmsImportTables) {
      ...
      const rows = normalizeImportRows(table, snapshot.tables[table] ?? []);
...
function normalizeImportRows(table, rows) {
...
function insertRow(db, table, tableColumns, row) {
  ...
  const ignoredLegacyColumns = getIgnoredLegacyColumns(table);
  const unknownColumns = Object.keys(row).filter((column) => !tableColumnSet.has(column) && !ignoredLegacyColumns.has(column));
...
function getIgnoredLegacyColumns(table) {
  if (table === 'cms_pages') {
    return new Set(['content_zh', 'seo_zh']);
  }

  if (table === 'cms_media') {
    return new Set(['alt_zh']);
  }

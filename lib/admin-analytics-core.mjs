...
export function analyticsPageCorrectionHref(filters, totalPages) {
  const lastPage = cappedAnalyticsTotalPages(totalPages);
  return filters.page > lastPage ? analyticsHref(filters, {page: lastPage}) : null;
}

export function cappedAnalyticsTotalPages(totalPages) {
  return Math.max(1, Math.min(MAX_ANALYTICS_PAGE, totalPages));
}

...
test('Creations keeps its newer dedicated mobile entrance image implementation', () => {
  assert.match(creationsPage, /hasMobileImage: Boolean\(filter\.mobileImage/);
  assert.match(creationsGallery, /mobileImage\?: string/);
  assert.match(creationsGallery, /src=\{imageSrc\(category\.mobileImage\)\}/);
});

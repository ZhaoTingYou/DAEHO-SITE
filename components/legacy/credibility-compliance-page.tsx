...
export function CredibilityCompliancePage({locale, content}: CredibilityCompliancePageProps) {
  ...
  return (
    <main className="mobile-page-shell bg-white text-primary">
      ...
      <section className="mobile-section relative z-10 bg-white md:py-[clamp(108px,12vw,176px)]">
      ...
      <section className="mobile-section relative z-10 bg-white md:px-container md:pb-[clamp(104px,10vw,154px)] md:pt-0">
        <div className="mx-auto max-w-[1120px] bg-white">
          {copy.rows.map((row, index) => (
            <Reveal
              key={row.title}
              className={`grid gap-6 border-[#b65c55]/45 py-10 md:grid-cols-[minmax(0,1fr)_minmax(240px,310px)] md:gap-[clamp(28px,4vw,52px)] md:py-[clamp(42px,5vw,66px)] md:items-center ${
                index < copy.rows.length - 1 ? 'border-b border-[#b65c55]/45' : ''
              }`}
            >
              <div className="order-1 md:col-start-1 md:row-start-1">
                <div className="max-w-[734px]">
                  <h2 className="mobile-display [font-family:'MaruBuri',serif] font-semibold tracking-normal text-primary md:text-[clamp(27px,2.5vw,32px)] md:leading-[1.22]">
              ...
              <div className="order-3 md:col-start-1 md:row-start-2">
                <div className="max-w-[734px]">
                  <p className="mt-[10px] whitespace-pre-line [font-family:'Pretendard',sans-serif] text-[clamp(18px,1.55vw,20px)] font-normal leading-[1.35] text-accent mobile-copy">
                    {row.accent}
                  </p>
                  <div className="mobile-copy mt-[18px] space-y-[14px]">
...
function CredibilityRowImage({image, alt}: {image?: string; alt: string}) {
  if (!image) {
    return <div className="mobile-media-landscape aspect-[4/3] w-full bg-[#d7d7d7]" aria-hidden="true" />;
  }

  return (
    <div className="mobile-media-landscape relative aspect-[4/3] w-full overflow-hidden bg-[#d7d7d7]">
      <Image
        src={imageSrc(image)}
        alt={alt}
        fill

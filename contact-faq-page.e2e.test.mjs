...
test('Contact renders the categorized C plus C1 FAQ structure with all answer copy in SSR HTML', {timeout: 30_000}, async (t) => {
  ...
  assert.equal((html.match(/data-contact-faq-reveal=/g) ?? []).length, 4);
  assert.equal((html.match(/data-contact-faq-extra-panel=/g) ?? []).length, 4);
  assert.equal((html.match(/data-contact-faq-extra-question=/g) ?? []).length, 8);
  assert.equal((html.match(/aria-controls="contact-faq-/g) ?? []).length, 28);

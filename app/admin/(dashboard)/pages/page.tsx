...
export default async function AdminPagesPage() {
  ...
  const groups = groupManagedPages();
      ...
      {managedPageDefinitions.length === 0 ? (
        ...
        <div className="grid gap-6">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <PageMetric label={t('page.metricEditable')} value={managedPageDefinitions.reduce((total, definition) => total + countEditableElements(definition, pagesByKey.get(definition.pageKey)?.content.ko), 0)} />
          </div>
            ...
            <Panel key={section} className="overflow-hidden">
              ...
              <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
                {definitions.map((definition) => {
                  ...
                  const fieldCount = countEditableElements(definition, page?.content.ko);
                    ...
                    <article key={definition.pageKey} className="grid min-h-[236px] grid-rows-[1fr_auto] rounded-md border border-[#e4e7ec] bg-[#fbfcfe] p-4">
                      <div>
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#98a2b3]">{definition.section}</p>
                            <h3 className="mt-2 font-heading text-xl font-semibold leading-tight text-[#101827]">{definition.title}</h3>
                          </div>
                          <StatusBadge active={Boolean(page)} activeLabel={t('page.initialized')} inactiveLabel={t('page.pendingInit')} />
                        </div>
                        <p className="mt-3 text-sm leading-6 text-[#647084]">{definition.description}</p>
...
function countEditableElements(definition: PageDefinition, pageContent: unknown) {
  const messages = getLocaleMessages('ko');
  ...
  return getPageContentGroups(definition).reduce((total, group) => {
    ...
    const content = deepMergeJson(staticContent, getPageContentGroupOverride(pageContent, group.key));

    return total + getEditableLeafCount(content);
  }, 0);
}

function groupManagedPages() {

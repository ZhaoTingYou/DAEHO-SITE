...
test('media upload form keeps the upload button reachable on tablet-sized admin screens', () => {
  ...
  assert.match(source, /xl:grid-cols-\[minmax\(0,1fr\)_minmax\(0,1fr\)_minmax\(0,1fr\)_auto\]/);
  assert.match(source, /md:col-span-2 xl:col-span-1/);
  assert.match(source, /className="admin-on-dark min-h-10 w-full/);
  assert.match(source, /<ContentLocalePanel locale="ko">[\s\S]*name="altKo"/);
  assert.match(source, /<ContentLocalePanel locale="en">[\s\S]*name="altEn"/);
});

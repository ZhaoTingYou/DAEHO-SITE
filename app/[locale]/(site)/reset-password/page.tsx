...
import {ResetPasswordForm} from '@/components/customer/reset-password-form';
...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {title: (await getAccountMessages(locale)).resetPassword.metadataTitle,
...
export default async function ResetPasswordPage({params}: {params: Promise<{locale: string}>}) {
  ...
  const copy = (await getAccountMessages(locale)).resetPassword;
  return (
    <main className="min-h-screen bg-bg px-container pb-24 pt-36 text-primary md:pt-44">
      <div className="mx-auto max-w-xl">
        ...
        <div className="mt-10 border-t border-hairline pt-8">
          {await accountsEnabled() ? <ResetPasswordForm locale={locale} copy={copy} /> : (

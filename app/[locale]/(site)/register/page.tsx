...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return {title: (await getAccountMessages(locale)).register.metadataTitle, robots: {index: false, follow: false}};
}
...
export default async function RegisterPage({params}: {params: Promise<{locale: string}>}) {
  ...
  const copy = (await getAccountMessages(locale)).register;
  return (
    <main className="min-h-screen bg-bg px-container pb-24 pt-36 text-primary md:pt-44">
      <div className="mx-auto max-w-xl">
        <p className="font-body text-xs font-semibold uppercase tracking-[0.2em] text-accent">{copy.eyebrow}</p>
        ...
        <div className="mt-10 border-t border-hairline pt-8">
          {await accountsEnabled() ? <RegisterForm locale={locale} copy={copy} /> : (

...
- 客户无需登录，以安全匿名浏览器凭证识别。
- 同一浏览器可以恢复当前 Bot 配置代次中最新一条咨询；这不是跨代次或多会话历史收件箱。该咨询自最后一次客户写入起最多保留 30 天，换设备或清除浏览器数据后视为新访客。
...
- 有效期只按客户创建咨询或发送追加消息滚动 30 天；团队回复、关闭事件、单纯打开页面、读取历史或 SSE 心跳都不延长有效期。团队回复受最后一次客户写入的 30 天边界约束。超过有效期后旧凭证失效，并仅在客户主动打开聊天窗或写入时创建新访客身份。
...
- 返回当前 Bot 配置代次中该 Cookie 所有的最新一条咨询、其分页可见历史、未读数量和功能可用状态；不提供跨代次或多个旧咨询的历史收件箱。
- 页面挂载时的无 Cookie 被动查询只返回空会话且不得创建访客或下发 Cookie；客户主动打开聊天窗时才允许签发身份。已有有效 Cookie 的被动查询可以读取其最新当前代次会话。
...
- 后台清理任务按最后一次客户写入关闭超过 30 天的活动咨询，发布持久关闭事件并记录可恢复的 Topic 关闭结果；每批有界地回收过期限流桶、已关闭匿名会话及无会话的过期访客。消息随匿名会话级联删除，但 CMS 问询记录不随匿名凭证过期而删除。
...
- Nginx 对表单、消息和 SSE 分别设置小请求体限制、无缓冲 SSE、合理连接超时、每 IP 请求速率和 SSE 连接数限制；应用层同时限制全局及单会话活动 emitter 数量。
...
- 服务端必须先按原始 `client_message_key` 解析已存在的创建/发送结果，再消耗限流或重复内容配额；`429` 之后客户端保留同一逻辑键，丢失响应后的重试不会复制消息。
- 客户气泡只由带正整数数据库 ID 的权威历史生成；刷新或重试按 ID 去重，不使用会破坏游标的合成 ID。
- 最高持久游标覆盖所有历史行，但未读数和已读游标只计算 `team` 消息。
- 历史使用显式 `nextCursor`/`hasMore` 续页和严格序列化字节预算，每个响应小于客户端 256 KiB 上限；合法多字节消息不截断，`session` 保持轻量且只带首页，客户端按持久 ID 去重并保序追页。
- 无有效所有者身份时，`session?issue=true` 和写入在创建 visitor 行之前先消耗应用层 IP 签发配额；新建会话还必须先通过 start IP 配额。已有合法所有者的原幂等键重放仍先解析并跳过配额。
- CMS 列表将所有 `pending_action='topic_close'` 的已关闭会话视为可操作项，不受当前配置代次或普通已关闭历史 50 条上限影响。
- 延迟的 Topic/注册恢复激活只改变状态与映射，不改写 `last_activity_at`；该时间仍只由客户写入续期，超过 30 天的延迟恢复对象应进入过期关闭。
- Telegram Webhook 使用 update/message ID 去重。
- 创建 Topic、发送初始卡片、追加客户消息和团队回复都先预留数据库状态，再执行外部调用。
- Telegram 返回明确失败时允许自动或 CMS 手动重试；访客投递预留使用两分钟租约，过期预留由精确键重放或每小时有界清理转为 `needs_attention`。CMS 只能按精确消息 ID 确认已送达，或以数据库中原始正文和幂等键重试；不确定投递禁止盲目复制消息或 Topic。
- CMS 的 Topic 重置确认动作必须在同一操作内继续安全创建；关闭会话必须先持久化唯一系统关闭事件并发布，再持久化 `topic_close` 成功或可重试失败状态，而不是仅记录日志。
- 多标签页共享同一 Cookie；数据库约束保证只有一个活动会话。标签页间可使用 `BroadcastChannel` 同步已读和关闭状态，但正确性不依赖浏览器广播。
- CMS 必须继续提供关闭、精确访客消息确认/重试、Topic 关闭重试，以及确认 Topic 不存在后立即重建等恢复操作。
...
- 打开、恢复或位于底部时的新消息自动滚动到底部；客户正在阅读旧记录时保持位置，并显示可聚焦、可读屏的“下方有新消息”操作。
- 键盘、焦点、屏幕阅读器和减少动态效果。

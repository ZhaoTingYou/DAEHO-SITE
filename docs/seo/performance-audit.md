...
### 4-0. 폰트 렌더링 차단 (효과 가장 큼 / **조치 완료**)

구글이 지목한 최대 원인입니다. 절감 가능 시간 **23,450ms**로, 다른 항목을 모두 합친 것보다 큽니다.
...
**후속 과제**: `next/font`로 전환하면 폰트를 자체 호스팅해 외부 도메인 의존을 완전히 없앨 수 있습니다. 다만 `Cormorant Garamond`를 직접 참조하는 코드가 **17개 파일 27곳**에 있어, 전환 시 모두 CSS 변수로 교체하고 시각 회귀를 검증해야 합니다. 별도 일정으로 진행하시길 권합니다.
...
| 항목 | 절감 가능 | 대응 |
|---|---|---|
| **렌더링 차단 요청** | **23,450ms** | **4-0에서 조치 완료** |
| 효율적인 캐시 수명 | 2,479KiB | 고객사 캐시 작업 (2장) |
...
이 수치는 **조치 전** 기준입니다. 4-0의 폰트 조치와 이미지 포맷 적용은 PR 머지 후 운영에 반영되므로, 개선 효과는 머지 이후에 측정할 수 있습니다.
...
| 항목 | 상태 |
|---|---|
| **폰트 렌더링 차단** | **조치 완료** — 절감 가능 23,450ms로 최대 원인이었음 |
| OG 이미지 | 3장 압축(88~91%)·파일명 개선 후 **운영 적용 완료** |
| 이미지 포맷 | AVIF·WebP **적용 완료** (머지 후 반영) |
...
| Brotli | 적용 검토 권고 |
| `next/font` 전환 | 후속 과제 (17개 파일 27곳 시각 검증 필요) |

조치 전 기준 성능 55점이며, **개선 효과는 PR 머지 후 재측정으로 확인할 수 있습니다.**

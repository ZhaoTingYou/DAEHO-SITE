...
`Dockerfile.next`로 프로덕션 이미지를 빌드해 컨테이너에서 응답 헤더를 확인했습니다.
...
**따라서 위 636ms는 캐시 적용 이전 수치이며 현재 상태를 나타내지 않습니다.** 다만 위 확인은 로컬 컨테이너 기준이므로, **실제 응답 속도는 AWS 배포 후 재측정이 필요합니다.**
...
### 4-3. 캐시 적용 (효과 큼 / **고객사 구현 완료**)

2장 참고. 고객사 개발팀이 최신 main에 구현을 반영했습니다. 효과는 배포 후 TTFB 재측정으로 확인합니다.
...
이 수치는 **조치 전** 기준입니다. 4-0의 폰트 요청 경로 개선, 이미지 포맷 적용, 고객사 캐시 구현은 모두 AWS 배포 후에야 운영에 반영되므로, 개선 폭은 배포 이후에만 측정할 수 있습니다.
...
| TTFB | 636ms는 캐시 적용 전 값. **고객사 캐시 구현 완료, 배포 후 재측정** |
| Brotli | 적용 검토 권고 |
| `next/font` 전환 | 후속 과제 — 폰트 차단의 완전 해결책 (17개 파일 27곳 시각 검증 필요) |

조치 전 기준 성능 55점입니다. **이 문서의 개선 항목은 어느 것도 아직 운영에 반영되지 않았으며, 실제 효과는 AWS 배포 후 재측정으로만 확인할 수 있습니다.**
